...
export const addToTimelineItems = (state, payload): ITimelineState => {
  ...
  return {
    ...state,
    appItems: [...state.appItems.filter((item) => !appIds.includes(item.id)), ...payload.appItems],
    logItems: [...state.logItems.filter((item) => !logIds.includes(item.id)), ...payload.logItems],
    statusItems: [
