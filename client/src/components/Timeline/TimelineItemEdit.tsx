...
export const TimelineItemEdit = memo<IProps>(
  ({ selectedTimelineItem, trayEdit, clearTimelineItem, saveTimelineItem, deleteTimelineItem }) => {
    ...
    const changeTime = (attr) => (value) => {
      ...
      const newDate = moment(value.toArray().slice(0, 4).concat(oldDate.toArray().slice(4)));
      setState({
        ...state,
        trackItem: {
          ...state.trackItem,
    ...
    if (trayEdit) {
      return (
        <Flex p={1} width={1}>
          ...
          <Box px={1}>
            <Button type="primary" shape="circle" icon={<CloseOutlined />} onClick={closeEdit} />
          </Box>
        </Flex>
      );
    }
    ...
    return (
      <Box width={600}>
        ...
        <Flex p={1} width={1}>
          ...
          <Flex px={1} width={2 / 3}>
            <Box pr={1}>
              <TimePicker value={moment(trackItem.beginDate)} onChange={changeTime('beginDate')} />
            </Box>
            <Box>
              <TimePicker value={moment(trackItem.endDate)} onChange={changeTime('endDate')} />
            </Box>
          </Flex>
        </Flex>
        <Divider />
        <Flex width={1}>
          ...
          <Box px={1}>
            <Button type="primary" icon={<SaveOutlined />} onClick={saveBasedOnColorOptionHandler}>
              Save
            </Button>
          </Box>
        </Flex>
