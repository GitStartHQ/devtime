...
export const TrackItemTable = () => {
          ...
          name: (
            <span>
              {record.title
                .split(reg)
                .map((text, i) =>
                  i > 0 ? [<Highlight key={text}>{match[0]}</Highlight>, text] : text,
                )}
            </span>
          ),
        };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Moment format={isOneDay ? TIME_FORMAT : DATE_TIME_FORMAT}>{record.beginDate}</Moment>
      ),
    },
    {
      title: 'End',
...

    {
      ...
      render: (text, record) => <span>{diffAndFormatShort(record.beginDate, record.endDate)}</span>,
    },
  ...
  return (
    <div>
      <Flex p={1}>
        <Box pr={1}>
          {hasSelected && (
            <Button type="primary" onClick={deleteSelectedItems} disabled={!hasSelected}>
