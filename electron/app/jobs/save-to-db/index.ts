...
export class SaveToDbJob {
    ...
    try {
      ...
      if (this.token) {
        ...
        for (const item of summary) {
          ...
          if (
            (item.type === 'task' && item.type === lastItem.type && item.id === lastItem.id) ||
            (item.type === 'ticket' && item.type === lastItem.type && item.id === lastItem.id) ||
            (item.type === 'client_project' &&
              item.type === lastItem.type &&
              item.id === lastItem.id) ||
            (item.type === 'client' && item.type === lastItem.type && item.id === lastItem.id)
          ) {
        ...
        if (grouppedSummary.length > 0) {
          try {
            ...
            if (
              ...
              ((grouppedSummary[0].type === 'task' &&
                ...
                (grouppedSummary[0].type === 'client' &&
                  grouppedSummary[0].type === this.lastSavedSummary.type &&
                  grouppedSummary[0].id.toString() === this.lastSavedSummary.id.toString()))
            ) {
              // mutation to update worklog endAt field
              const update = await fetchGraphQLClient(process.env.HASURA_GRAPHQL_ENGINE_DOMAIN, {
                token: this.token,
              })<
                {
              ...
              if ((update.errors?.length ?? 0) > 0) {
                if (
                  ...
                  throw new Error(`Error updating user worklog | ${JSON.stringify(update.errors)}`);
                }
              }
            ...
            if (grouppedSummary.length > 0) {
              ...
              const insert = await fetchGraphQLClient(process.env.HASURA_GRAPHQL_ENGINE_DOMAIN, {
                token: this.token,
              })<
                {
              ...
              >({
                ...
                variables: {
                  worklogs: grouppedSummary.map((summary) => ({
                    ...
                    ...(summary.type === 'client_project' ? { clientProjectId: summary.id } : {}),
              ...
              if ((insert.errors?.length ?? 0) > 0) {
                ...
                if (
                  ...
                  throw new Error(
                    `Error inserting user worklog | ${JSON.stringify(insert.errors)}`,
                  );
                }
              }

              const returningLength = insert.data.insert_user_work_logs.returning.length;
              this.lastSavedSummary = {
                ...grouppedSummary[grouppedSummary.length - 1],
                worklogId: insert.data.insert_user_work_logs.returning[returningLength - 1].id,
              };
            }
          } catch (err) {
  ...
  private async saveUserEvents() {
    try {
      ...
      if (this.token) {
        ...
        await Promise.all(
          returned.data.insert_user_events.returning.map((userEvent, i) => {
            ...
            return trackItemService.updateTrackItem({ userEventId: userEvent.id }, items[i].id);
          }),
        );
      ...
      if (!!stagingUserId) {
        console.log(items.length, `TrackItems need to be upserted to GitStart's Staging DB`);
        ...
        if ((returnedFromStaging.errors?.length ?? 0) > 0) {
          ...
          console.log('Successfully saved', items.length, `TrackItems to GitStart's Staging DB`);
        }
      }
    } catch (e) {
      logErrors(e);
