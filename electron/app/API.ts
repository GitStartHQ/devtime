...
const settingsActions = {
  ...
  loginInExternalBrowser: async () => {
    shell.openExternal(`${process.env.GITSTART_LOGIN_URL}?url=${appConstants.PROTOCOL_NAME}://`);
  },
...
const trackItemActions = {
  findAllDayItems: async ({ payload }, res) => {
    const data = await trackItemService.findAllDayItems(payload.from, payload.to, payload.taskName);
  ...
  updateTrackItem: async ({ payload }, res) => {
    const data = await trackItemService.updateTrackItem(payload.trackItem, payload.trackItem.id);
    res.send(data);
  },
